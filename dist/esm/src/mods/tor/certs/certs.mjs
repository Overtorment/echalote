...
import { RsaWasm, RsaPublicKey } from '../../../libs/rsa/index.mjs';
...
(function (Certs) {
    ...
    async function verifyRsaToEdOrThrow(certs) {
        ...
        try {
            ...
            const publicKeyMemory = __addDisposableResource(env_1, new RsaWasm.Memory(publicKeyBytes), false);
            const publicKeyPointer = __addDisposableResource(env_1, RsaPublicKey.from_public_key_der(publicKeyMemory), false);
            ...
            const hashedMemory = __addDisposableResource(env_1, new RsaWasm.Memory(hashed), false);
            const signatureMemory = __addDisposableResource(env_1, new RsaWasm.Memory(certs.rsa_to_ed.signature), false);
    ...
    async function verifyEdToSigningOrThrow(certs) {
        ...
        try {
            ...
            const identity = __addDisposableResource(env_2, await Ed25519.get().getOrThrow().VerifyingKey.importOrThrow(certs.rsa_to_ed.key), false);
            const signature = __addDisposableResource(env_2, Ed25519.get().getOrThrow().Signature.importOrThrow(certs.ed_to_sign.signature), false);
    ...
    async function verifySigningToTlsOrThrow(certs, tlsCerts) {
        ...
        try {
            ...
            const identity = __addDisposableResource(env_3, await Ed25519.get().getOrThrow().VerifyingKey.importOrThrow(certs.ed_to_sign.certKey), false);
            const signature = __addDisposableResource(env_3, Ed25519.get().getOrThrow().Signature.importOrThrow(certs.sign_to_tls.signature), false);
...
//# sourceMappingURL=certs.mjs.map
