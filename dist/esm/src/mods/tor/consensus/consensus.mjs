...
import { asOpaqueDuplex } from '../stream.mjs';
...
(function (Consensus) {
        ...
        for (const it of consensus.signatures) {
            if (it.algorithm === "sha256") {
                ...
                try {
                    ...
                    const signingKey = __addDisposableResource(env_1, Base64.get().getOrThrow().decodePaddedOrThrow(certificate.signingKey), false);
                    ...
                    const signature = __addDisposableResource(env_1, Base64.get().getOrThrow().decodePaddedOrThrow(it.signature), false);
                    const signatureM = __addDisposableResource(env_1, new RsaWasm.Memory(signature.bytes), false);
                    const hashedM = __addDisposableResource(env_1, new RsaWasm.Memory(hashed), false);
                    const publicKeyM = __addDisposableResource(env_1, new RsaWasm.Memory(publicKey), false);
                    const publicKeyX = __addDisposableResource(env_1, RsaWasm.RsaPublicKey.from_public_key_der(publicKeyM), false);
    ...
    (function (Certificate) {
        ...
        async function verifyOrThrow(cert) {
            ...
            try {
                const identityKey = __addDisposableResource(env_2, Base64.get().getOrThrow().decodePaddedOrThrow(cert.identityKey), false);
                ...
                const subjectPublicKeyInfo = new X509.SubjectPublicKeyInfo(algorithmId, subjectPublicKey);
                const publicKey = X509.writeToBytesOrThrow(subjectPublicKeyInfo);
                const signature = __addDisposableResource(env_2, Base64.get().getOrThrow().decodePaddedOrThrow(cert.signature), false);
                const hashedM = __addDisposableResource(env_2, new RsaWasm.Memory(hashed), false);
                const publicKeyM = __addDisposableResource(env_2, new RsaWasm.Memory(publicKey), false);
                const signatureM = __addDisposableResource(env_2, new RsaWasm.Memory(signature.bytes), false);
                const publicKeyX = __addDisposableResource(env_2, RsaWasm.RsaPublicKey.from_public_key_der(publicKeyM), false);
                const verified = publicKeyX.verify_pkcs1v15_unprefixed(hashedM, signatureM);
                if (verified !== true)
                    throw new Error(`Could not verify`);
                return true;
...
export { Consensus };
//# sourceMappingURL=consensus.mjs.map
